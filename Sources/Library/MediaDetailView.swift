// ==========================================================
//  BUG 3  -  GROUP ONCE, NOT EIGHT TIMES A FRAME  (1 of 4)
//
//  File:  Sources/Models/MediaItem.swift
//  Replace the entire file. Supersedes IMP-4a.
//
//  Series.seasons was a computed property that rebuilt a grouping
//  dictionary on every read, and the detail views read it several times
//  per frame. It is stored now, built once in the initialiser.
//
//  The Array.series(withID:) helper is gone. It walked the whole library,
//  regrouping it, to find one show. LibraryStore keeps a dictionary now.
// ==========================================================
...
struct Series: Identifiable {
    ...
    /// Stored, not computed. This used to rebuild a grouping dictionary on every
    /// read, and the detail views read it several times per frame.
    let seasons: [Season]

    init(id: String, title: String, posterURL: URL?, backdropURL: URL?, overview: String,
         releaseYear: String, rating: Double?, genres: [String], cast: [CastMember],
         episodes: [MediaItem]) {
        self.id = id
        self.title = title
        ...
        self.seasons = Self.buildSeasons(from: episodes, showPoster: posterURL)
    }

    private static func buildSeasons(from episodes: [MediaItem], showPoster: URL?) -> [Season] {
        Dictionary(grouping: episodes, by: { $0.seasonNumber })
            .map { number, eps in
                ...
                return Season(
                    ...
                    posterURL: eps.compactMap { $0.metadata?.seasonPosterURL }.first ?? showPoster,
                    episodes: sorted
                )
            }
