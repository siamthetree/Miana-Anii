    ...
    var body: some Scene {
        WindowGroup {
            LibraryView()
                ...
                .tint(.purple)
                .task {
                    await TraktService.shared.refreshIfNeeded()
                }
                .onOpenURL { url in
                    if url.scheme == "minaanii" {
                        Task { @MainActor in await TraktService.shared.handleAuthRedirect(url: url) }
                    } else {
                        Task { @MainActor in await store.importFile(url); store.save() }
                    }
                }
        }
    }
...
final class TraktService: ObservableObject {
    ...
    func logout() {
        ...
        Keychain.delete(Key.access); Keychain.delete(Key.refresh); Keychain.delete(Key.expiry)
        isAuthenticating = false
    }
...
struct TMDBResult: Codable {
    ...
    let poster_path: String?; let backdrop_path: String?; let release_date: String?; let first_air_date: String?
}
...
final class MetadataService {
    ...
    static func fetchMetadata(for filename: String, cleanTitle: String) async -> MediaMetadata? {
        ...
        if isTV {
            ...
            if let regex = try? NSRegularExpression(pattern: pattern), let match = regex.firstMatch(in: filename, range: NSRange(location: 0, length: filename.utf16.count)) {
                if let sRange = Range(match.range(at: 1), in: filename), let eRange = Range(match.range(at: 2), in: filename) {
                    season = Int(filename[sRange]); episode = Int(filename[eRange])
                }
            }
        }
        ...
        if let regex = try? NSRegularExpression(pattern: yearPattern),
           let match = regex.firstMatch(in: cleanTitle, range: NSRange(location: 0, length: cleanTitle.utf16.count)) {
            if let yearRange = Range(match.range, in: cleanTitle) {
                explicitYear = String(cleanTitle[yearRange])
                searchTitle = cleanTitle.replacingOccurrences(of: explicitYear!, with: "").trimmingCharacters(in: .whitespacesAndNewlines)
            }
        }
        ...
        do {
            ...
            var rating: Double? = nil; var genres: [String] = []; var cast: [String] = []
...
final class LibraryStore: ObservableObject {
    ...
    static func prettyTitle(from raw: String) -> String {
        ...
        if let regex = try? NSRegularExpression(pattern: pattern) {
            s = regex.stringByReplacingMatches(in: s, range: NSRange(location: 0, length: s.utf16.count), withTemplate: "").trimmingCharacters(in: .whitespacesAndNewlines)
        }
